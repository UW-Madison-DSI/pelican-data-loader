...
def render_metadata_input(state: TypedSessionState):
    ...
    with col1:
        dataset_name = st.text_input("Dataset Name", value=state.dataset_info.name, help="A human-readable name for the dataset")
        dataset_description = st.text_area(
            "Dataset Description",
            value=state.dataset_info.description,
            help="A detailed description of the dataset",
        )
        dataset_version = st.text_input("Version", value=state.dataset_info.version, help="Version of the dataset, e.g., '1.0.0'")
        cite_as = st.text_input("Citation", value=state.dataset_info.cite_as, help="How to cite this dataset")
    ...
    with col2:
        ...
        license_url = st.selectbox("License", options=LICENSES, help="Choose a license for the dataset", index=license_index)
